...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    return NextResponse.json(routes.map((r) => routeToApi(r as FullRoute)));
  } catch (err) {
    console.error("GET /api/regions/[id]/routes error:", err);
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const route = await prisma.trekRoute.create({
      data: {
        slug,
        title,
    ...
    return NextResponse.json(routeToApi(route as FullRoute), { status: 201 });
  } catch (err) {
    console.error("POST /api/regions/[id]/routes error:", err);
