  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex gap-4 mb-5 text-sm text-gray-500">
        ...
        <span>·</span>
        <span>
          {guides.filter((g) => g.availabilityStatus === "AVAILABLE").length} available
        </span>
        <span>·</span>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                {["Guide", "Specialty", "Region", "Rate", "Status", "Trust", ""].map((h) => (
                  <th
                    key={h}
                    className="text-left px-4 first:px-5 py-3.5 text-[10px] uppercase tracking-widest text-gray-500 font-semibold"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((guide) => {
                const status = STATUS_BADGE[guide.availabilityStatus];
                  ...
                  <tr key={guide.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5">
                        {guide.kycVerified && (
                          <span aria-label="KYC Verified">
                            <Shield className="w-4 h-4 text-emerald-400" />
                          </span>
                        )}
                        {guide.isVerified && (
                          <span aria-label="Platform Verified">
